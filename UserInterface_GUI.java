    ...
    *****************************************************************/       
    private class EditEmployeePanel extends JPanel implements ActionListener
    {
        ...
        public EditEmployeePanel()
        {
            ...
            lblWage.setPreferredSize(new Dimension(100, 30));
            gbc.gridx = 0;
            gbc.gridy = 4;
            gbc.anchor = GridBagConstraints.WEST;
            gbLayout.setConstraints(lblWage, gbc);
            this.add(lblWage);
            
            tbWage = new JTextField(4);

            gbc.gridx = 1;
            gbc.gridy = 4;
            gbLayout.setConstraints(tbWage, gbc);
            this.add(tbWage);
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == btnOK)
            {
                ...
                int Wage =  Integer.parseInt(tbWage.getText());
                
                if(isUpdate)
                {
                    if(! rcController.updateStaff(staffID , new String(tbPassword.getPassword()), tbFirstName.getText(), tbLastName.getText(),Wage)) 
                    {
                        showErrorDialog("Error", rcController.getErrorMessage());
                        return;
                    }
            ...
            else if (ae.getSource() == btnCloseOrder)
            {
                ...
                if( showYesNoDialog("Close order","Are you sure to close the order?") == DIALOG_YES)
                {
                    if( !rcController.closeOrder(orderID))
                        ...
                        JFrame frame = new JFrame("Review Dialog Box Example");
                        String review = JOptionPane.showInputDialog(frame, "Enter your review:");
                        rcController.addReview(orderID,review);
